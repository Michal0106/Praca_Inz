...
class StravaService {
 ...
 async getActivityStreams(accessToken, activityId, keys = ['time', 'latlng', 'distance', 'altitude', 'velocity_smooth', 'heartrate', 'watts', 'cadence']) {
 try {
 ...
 console.error('Strava get streams error:', error.response?.data || error.message);
 throw error;
 }
 }
}
