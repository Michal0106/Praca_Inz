...
import { Link2, Unlink, RefreshCw, User, Mail, Calendar, CheckCircle } from 'lucide-react';
...
function AccountPage() {
 ...
 // Sprawdź czy wróciliśmy z OAuth
 ...
 // Auto-sync po połączeniu ze Stravą
 ...
 // Przekieruj do Strava OAuth z tokenem w query parameter
 ...
 alert(`Zsynchronizowano aktywności ze Stravą!\n\nNowe aktywności: ${response.data.newActivitiesCount}`);
 } catch (error) {
 console.error('Auto-sync error:', error);
 alert('Błąd podczas automatycznej synchronizacji. Możesz spróbować ręcznie z Dashboard.');
 } finally {
 setSyncing(false);
 }
 };
 ...
 if (!confirm('Czy na pewno chcesz odłączyć konto Strava? Utracisz dostęp do synchronizacji danych.')) {
 return;
 }
 ...
 alert('Błąd podczas odłączania konta Strava: ' + (error.response?.data?.error || error.message));
 }
 };
 ...
 <p>Twoje konto Strava zostało połączone. Możesz teraz synchronizować aktywności.</p>
 </div>
 </div>
 )}
 ...
 <p>{new Date(user?.createdAt || Date.now()).toLocaleDateString('pl-PL')}</p>
 </div>
 </div>
 </div>
 </div>
 ...
 Możesz teraz synchronizować swoje aktywności z panelu głównego
 </p>
 </div>
 <button 
 ...
 <>
 <div className="connection-details">
 <p> Połącz swoje konto Strava, aby synchronizować aktywności</p>
 <p className="text-muted">
 Po połączeniu będziesz mógł automatycznie importować wszystkie swoje treningi
 </p>
 </div>
 <button 
 ...
 {linking ? 'Przekierowywanie...' : 'Połącz z kontem Strava'}
 </button>
 </>
 )}
 </div>
 ...
 {/* Placeholder dla przyszłych połączeń */}
 ...
 <span className="stat-value">{user.stats.totalActivities || 0}</span>
